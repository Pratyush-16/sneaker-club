...
import { removeFromCart, updateQuantityInCart } from "../../Services/CartServices";
      ...
      <div className="cart-container">
        {state.cart?.map((item) => (
          <div className="category-cardd">
            ...
            <p>Qty: {item.qty}</p>
            <button onClick={() => updateQuantityInCart(item._id,token,dispatch,"increment")}>+</button>

            <button onClick={()=> updateQuantityInCart(item._id,token,dispatch,"decrement")}  disabled={item.qty ===1}> - </button>
            ...
            <button className="cart-Btn" onClick={()=> {addToWishlist(item,token, dispatch) 
               removeFromCart(item._id,token,dispatch)} }>Add to Wishlist</button> 
              </div>
