...
export const reducer = (state, action) => {
  switch (action.TYPE) {
    ...
    case "checkbox": {
      const isPresent = state.filter.categorySelected.find(
      ...
      if (isPresent) {
        return {
          ...
          filter: {
            ...state.filter,
            categorySelected: state.filter.categorySelected.filter(
        ...
        return {
          ...
          filter: {
            ...state.filter,
            categorySelected: [...state.filter.categorySelected, action.value],
          },
        };
      }
    }
    ...
    case "checkbox-brand": {
      const isPresent = state.filter.brand.find(
        (category) => category === action.value
      );
      ...
      if (isPresent) {
        return {
          ...
          filter: {
            ...state.filter,
            brand: state.filter.brand.filter((name) => name !== action.value),
          },
        };
      } else {
        return {
          ...
          filter: {
            ...state.filter,
            brand: [...state.filter.brand, action.value],
          },
        };
      }
    }
    ...
    case "REMOVE_ADDRESS":
      ...
      return {
        ...state,
        address: state.address.filter(({ _id }) => _id !== action.payLoad),
      };
